...
int GetTrentnaGodina() {
	...
	return 1900 + lokalnoVrijeme.tm_year;
...
string GenerisiSifru(string imeprezime, int id) {
	...
	int trenutnaGodina = GetTrentnaGodina();
	...
	sifra += ":";
	sifra += to_string(trenutnaGodina);
	sifra += "-";
...
bool ValidirajSifru(string sifra) {
	...
	regex uzorak("[A-Z]{2}:[0-9]{4}-[0-9]{3}");

	return regex_match(sifra, uzorak);
...
// a vraca novo stanje kolekcije tj. kolekciju zajedno sa novododatim elementom
	...
	KolekcijaParova* UkloniRaspon(int lokacijaOd, int brojElemenata) {
		...
		KolekcijaParova* uklonjeniProizvodi = new KolekcijaParova();
...
public:
	...
	bool operator == (const Proizvod& obj) {
...
public:
	...
	virtual Transakcija* Kloniraj() const = 0;
...
public:
	...
	Transakcija* Kloniraj() const override {
...
public:
	...
	Kupac(const Kupac& obj) {
		...
		for (int i = 0; i < obj._transakcije.size(); i++)
		{
			...
			//Kupovina* castedKupovina = dynamic_cast<Kupovina*>(obj._transakcije[i]);
			//Povrat* castedPovrat = dynamic_cast<Povrat*>(obj._transakcije[i]);

			//if (castedKupovina) {

			//	_transakcije.push_back(new Kupovina(*castedKupovina));
			//}
			//else {

			//	_transakcije.push_back(new Povrat(*castedPovrat));

			//}

			_transakcije.push_back(obj._transakcije[i]->Kloniraj());
...
// metoda vraca true ili false
	...
	bool DodajTransakciju(Kupovina kupovina) {
		...
		for (int i = 0; i < _transakcije.size(); i++)
		{
			...
			// proci kroz sve proizvode te transakcije

			// TRANSAKCIJA* -> KUPOVINA* 

			Kupovina* kupovinaTransakcija = dynamic_cast<Kupovina*>(_transakcije[i]);

			if (kupovinaTransakcija) { // da bi se sprijecilo da tranaskicja bude Povrat jer u tom slcuaju bi ovo puklo

				// prosli smo kroz sve proizvode nasih kupovine

				for (int j = 0; j < kupovinaTransakcija->GetProizvodi().size(); j++)
				{

					// prosli smo kroz sve proizvode kupovine koje neko zeli dodati
...
public:
	...
	void DodajKupca(Kupac kupac) {
		...
		{
			if (kupac == _kupci[i]) {
				throw exception("Kupac je vec dodat");
			}
		}
	...
	bool RegistrujTransakcijuKupcu(const char* sifraKupca, Kupovina kupovina) {
		...
		for (int i = 0; i < _kupci.size(); i++)
		{
			...
			// pronaci kupca sa sifrom koja nam je data
	...
	KolekcijaParova<Kupac, int, 50> PotrosnjaPoKategoriji(Kategorija kategorija) {
		...
		for (int i = 0; i < _kupci.size(); i++)
		{
			...
			for (int j = 0; j < _kupci[i].GetTransakcije().size(); j++)
			{
				...
				if (kupovinaTransakcija) {

					// prosli smo kroz sve proizvode odabrane transakcije Kupovine

					for (int k = 0; k < kupovinaTransakcija->GetProizvodi().size(); k++)
					{

						// provjeriti da li proizvod pripada kategoriji ELEKTRONIKA
	...
	friend ostream& operator << (ostream& COUT, const Prodavnica& obj) {
		...
		COUT << "Naziv prodavnice: " << obj._naziv << endl;

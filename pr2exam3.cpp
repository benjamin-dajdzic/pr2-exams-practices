...
string GenerisiClanskiBroj(string imePrezime, int redniBroj) {
	...
	char prvoSlovoPrezime = toupper(imePrezime[imePrezime.find(' ') + 1]);

	string sifra;

	sifra += to_string(GetTrenutnaGodina());
	sifra += "/";
	sifra += prvoSlovoIme;
	sifra += prvoSlovoPrezime;
	sifra += "-";
...
bool ValidirajClanskiBroj(const string& clanskiBroj) {
	...
	regex pattern("[0-9]{4}[/][A-Z]{2}[-][0-9]{3}");

	return regex_match(clanskiBroj, pattern);
...
public:
	...
	friend ostream& operator<< (ostream& COUT, const Datum& obj) {
		COUT << setw(2) << setfill('0') << *obj._dan << "." << setw(2) << setfill('0') << *obj._mjesec << "." << *obj._godina;
		return COUT;
	}
...
class Posudba {
	...
	int _brojDana;
public:
	Posudba(const char* naslov, Zanr zanr, Datum datumPosudbe, int brojDana) : _zanr(zanr), _datumPosudbe(datumPosudbe), _brojDana(brojDana) {
	...
	bool operator == (const Posudba& obj) {
	...
	friend ostream& operator << (ostream& COUT, const Posudba& obj) {
...
public:
	...
	bool DodajPosudbu(Posudba posudba) {
		...
		if (_posudbe.size() >= 3 || posudba.GetBrojDana() < 0 || posudba.GetBrojDana() > 30) {
			return false;
		}
	...
	bool RazduziKnjigu(const char* naslov) {
		...
		for (int i = 0; i < _posudbe.size(); i++)
		{
			if (strcmp(_posudbe[i].GetNaslov(), naslov) == 0) {

				_posudbe.erase(_posudbe.begin() + i); // iterator + pozicija
	...
	bool operator == (const ClanBiblioteke& obj) {
...
public:
	...
	bool EvidentirajPosudbu(const char* clanskiBroj, Posudba posudba) {
		...
		for (int i = 0; i < _clanovi.size(); i++)
		{
			if (strcmp(clanskiBroj, _clanovi[i].GetClanskiBroj()) == 0) {
				...
				return _clanovi[i].DodajPosudbu(posudba);
	...
	Kolekcija<ClanBiblioteke, int, 50> PosudbePoZanru(Zanr zanr) {
		...
		for (int i = 0; i < _clanovi.size(); i++)
		{
			...
			bool imaPosudbu = false;
			int brojPosudbiPoZanru = 0;
			...
			if (imaPosudbu) {
				strucnePosudbe.Dodaj(_clanovi[i], brojPosudbiPoZanru);
			}
	...
	bool operator == (const Biblioteka& obj) {
...
int main() {
	...
	if (!ValidirajClanskiBroj("2026/Ab-003") && !ValidirajClanskiBroj("26/AB-003") &&
	...
	try {
		...
		prosireniInventar.DodajNaPoziciju(10, 98, "Greska izdanje");
	}
	catch (exception& e) {
